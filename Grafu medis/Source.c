...
void printToFiles(char *text, FILE *fw) {
    printf("%s", text);
    fprintf(fw, "%s", text);
}

void BFS(struct Node* startNode, int *Result, int *rn, int *Depth, FILE *fw) {
    ...
    fprintf(fw, "DALIS 2. Vykdymas:\n");
        ...
        printf("  T=%d. Busena %d:\n", T, T); fprintf(fw, "  T=%d. Busena %d:\n", T, T);
        printf("          Aplankytu virsuniu aibe: {"); fprintf(fw, "          Aplankytu virsuniu aibe: {");
            for (int i = 0; i < rear; i++) {
                if (i == 0)
                {
                    printf("%d", queue[i]->label); fprintf(fw, "%d", queue[i]->label);
                    first = false;
                }
                else
                {
            ...
            if (front == rear)
            {
                printf("          Virsuniu eiles aibe: tuscia aibe\n"); fprintf(fw, "          Virsuniu eiles aibe: tuscia aibe\n");
            }
            else
            {
                printf("          Virsuniu eiles aibe: {"); fprintf(fw, "          Virsuniu eiles aibe: {");
    ...
    while (front < rear) {
        ...
        printf("  T=%d. Veiksmas %d:\n", T+1, T+1); fprintf(fw, "  T=%d. Veiksmas %d:\n", T+1, T+1);
        printf("          Aplankomu virsuniu aibe: {"); fprintf(fw, "          Aplankomu virsuniu aibe: {");
        for (int i = 0; i < currentNode->numNeighbors; i++) {
            ...
            if (neighbor != NULL && !neighbor->visited) {
                ...
                if (first == true)
                {
                    printf("%d", neighbor->label); fprintf(fw, "%d", neighbor->label);
                    first = false;
                }
                else
                {
        ...
        printf("          Virsuniu istatomu i eile aibe: {"); fprintf(fw, "          Virsuniu istatomu i eile aibe: {");
        for (int i = 0; i < currentNode->numNeighbors; i++) {
            struct Node* neighbor = currentNode->neighbors[i];
            if (neighbor != NULL && !neighbor->visited) {
                queue[rear++] = neighbor;
    ...
    /* printf("  T=% 2d. Busena %d:\n", T, T); fprintf(fw, "  T=% 2d. Busena %d:\n", T, T);
    ++T;
    printf("          Aplankytu virsuniu aibe: {"); fprintf(fw, "          Aplankytu virsuniu aibe: {");
    for (int i = 0; i < rear; i++) {
        if (i == 0)
...
int main() {
    ...
    BFS(Nodes[0], Result, &rn, Depth, fw);
    ...
    for (int i = 0; i < rn; i++) {
        ...
        if (i == 0)
        {
            printf("%d", Result[i]); fprintf(fw, "%d", Result[i]);
        }
        else
        {
            printf(", %d", Result[i]); fprintf(fw, ", %d", Result[i]);
